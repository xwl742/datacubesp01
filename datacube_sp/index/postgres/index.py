...
from datacube_sp.drivers.postgres import PostgresDb, PostgresDbAPI
from datacube_sp.index.postgres._transaction import PostgresTransaction
from datacube_sp.index.postgres._datasets import DatasetResource  # type: ignore
from datacube_sp.index.postgres._metadata_types import MetadataTypeResource
from datacube_sp.index.postgres._products import ProductResource
from datacube_sp.index.postgres._users import UserResource
from datacube_sp.index.abstract import AbstractIndex, AbstractIndexDriver, default_metadata_type_docs, AbstractTransaction
...
class Index(AbstractIndex):
    """
    Access to the datacube_sp index.

    DON'T INITIALISE THIS DIRECTLY (it will break in the future). Use `datacube_sp.index.index_connect()` or
    access property ``.index`` on your existing :class:`datacube_sp.api.core.Datacube`.
    ...
    :ivar datacube_sp.index._datasets.DatasetResource datasets: store and retrieve :class:`datacube_sp.model.Dataset`
    :ivar datacube_sp.index._products.ProductResource products: store and retrieve :class:`datacube_sp.model.DatasetType`\
    (should really be called Product)
    :ivar datacube_sp.index._metadata_types.MetadataTypeResource metadata_types: store and retrieve \
    :class:`datacube_sp.model.MetadataType`
    :ivar UserResource users: user management

    :type users: datacube_sp.index._users.UserResource
    :type datasets: datacube_sp.index._datasets.DatasetResource
    :type products: datacube_sp.index._products.ProductResource
    :type metadata_types: datacube_sp.index._metadata_types.MetadataTypeResource
    """
