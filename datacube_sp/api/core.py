...
from datacube_sp.storage import reproject_and_fuse, BandInfo, BandInfo_sp, RasterDataSourceforGDAL
from datacube_sp.utils import ignore_exceptions_if
from datacube_sp.utils import geometry
from datacube_sp.utils.dates import normalise_dt
from datacube_sp.utils.geometry import intersects, GeoBox
from datacube_sp.utils.geometry.gbox import GeoboxTiles
from datacube_sp.model import ExtraDimensions
...
class Datacube(object):
    """
    Interface to search, read and write a datacube_sp.

    :type index: datacube_sp.index.index.Index
    """
    ...
    def __init__(self,
        ...
        :type index: :py:class:`datacube_sp.index.Index` or None.
        ...
        :param str env: Name of the datacube_sp environment to use.
            ie. the section name in any config files. Defaults to 'datacube_sp' for backwards
            compatibility with old config files.

            Allows you to have multiple datacube_sp instances in one configuration, specified on load,
    ...
    def list_products(self, with_pandas=True, dataset_count=False):
        """
        List all products in the datacube_sp. This will produce a ``pandas.DataFrame``
        ...
        :return: A table or list of every product in the datacube_sp.
        :rtype: pandas.DataFrame or list(dict)
        """
        # Read properties from each datacube_sp product
        cols = [
            'name',
            'description',
            'license',
    ...
    def load(self, product=None, measurements=None, output_crs=None, resolution=None, resampling=None,
        ...
        **Dimensions**
            ...
            See :func:`datacube_sp.helpers.ga_pq_fuser` for an example implementation.
            see :func:`datacube_sp.api.query.query_group_by` for `group_by` built-in functions.
        ...
        **Output**
        ...
        :param xarray.Dataset like:
            ...
            resolution (i.e. :class:`datacube_sp.utils.geometry.GeoBox`).
            E.g.::
        ...
        :param datasets:
            Optional. If this is a non-empty list of :class:`datacube_sp.model.Dataset` objects, these will be loaded
        ...
        :param function dataset_predicate:
            ...
            take a :class:`datacube_sp.model.Dataset` object (e.g. as returned from :meth:`find_datasets`) and
            return a boolean.
    ...
    def find_datasets(self, **search_terms):
        ...
        :param search_terms: see :class:`datacube_sp.api.query.Query`
    ...
    def find_datasets_lazy(self, limit=None, ensure_location=False, dataset_predicate=None, **kwargs):
        ...
        :param kwargs: see :class:`datacube_sp.api.query.Query`
        ...
        :rtype: __generator[:class:`datacube_sp.model.Dataset`]
    ...
    def create_storage(coords, geobox, measurements, data_func=None, extra_dims=None):
        ...
        This function makes the in memory storage structure to hold datacube_sp data.
        ...
        :param measurements:
            list of :class:`datacube_sp.model.Measurement`
    ...
    def load_data(sources, geobox, measurements, resampling=None,
        ...
        :param xarray.DataArray sources:
            DataArray holding a list of :class:`datacube_sp.model.Dataset`, grouped along the time dimension
