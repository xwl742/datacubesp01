...
import datacube_sp
from datacube_sp.index.exceptions import IndexSetupError
from datacube_sp.utils import jsonify_document
...
_LIB_ID = 'odc-' + str(datacube_sp.__version__)
...
class PostGisDb(object):
        ...
        if validate:
            if not _core.database_exists(engine):
                raise IndexSetupError('\n\nNo DB schema exists. Have you run init?\n\t{init_command}'.format(
                    init_command='datacube_sp system init'
                ))
            ...
            if not _core.schema_is_latest(engine):
                raise IndexSetupError(
                    ...
                    'An administrator must run init:\n\t{init_command}'.format(
                        init_command='datacube_sp -v system init'
                    ))
    ...
    def _create_engine(url, application_name=None, iam_rds_auth=False, iam_rds_timeout=600, pool_timeout=60):
        ...
        if iam_rds_auth:
            from datacube_sp.utils.aws import obtain_new_iam_auth_token
    ...
    def create_spatial_index(self, crs: "datacube_sp.utils.geometry.CRS") -> Optional[Type[SpatialIndex]]:
        """
